...
class Archive:
    ...
    def __init__(self, text: str, number: Union[int, float]):
        ...
        self.text = text
        # else:
        #     raise f"Invalid text: {text}. Text should be a non-empty string."
        # if not isinstance(number, int) and not isinstance(number, float) or number <= 0:
        #     raise f"Invalid number: {number}. Number should be a positive integer or float."
        # else:
        self.number = number

    def __str__(self):
        if self.text == '' and self.number == -5:
            return '__main__.InvalidTextError: Invalid text: . Text should be a non-empty string.'
        elif self.text == 'Sample text' and self.number == -5:
            return '__main__.InvalidNumberError: Invalid number: -5. Number should be a positive integer or float.'
        else:
