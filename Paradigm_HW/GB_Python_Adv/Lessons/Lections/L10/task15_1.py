...
class Hero(Person):
    ...
    def add_many_up(self):
        self.up += 1
        self.up = min(self.up, self._Person__max_up * 2)   # Скорее всего переменная не д/б быть приватной (рам. прогр)
...
print(f'{p1.health = }, {p2.health = }')  # дочерние классы переопределяют методы родительского
