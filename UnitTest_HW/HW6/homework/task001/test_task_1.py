...
@pytest.fixture
def list2():
    """ Фикстура, возвращающая список со значениями [7, 6, 5, 4, 3]. """
    return [7, 6, 5, 4, 3]
...
def test_init(list1, list2):
    ...
    assert nums_list.ls1 == list1
    assert nums_list.ls2 == list2
...
@pytest.mark.parametrize('ls1, ls2, result', [([1, 2, 3], [], (2, 0)),
                                                ([], [1, 2, 3], (0, 2)), ([], [], (0, 0))])
def test_get_empty_lists_averages(ls1, ls2, result):
    """ Проверка средних значений, если один или оба списка пустые. """
    nums_list = NumsLists(ls1, ls2)
    assert nums_list.get_lists_averages() == result


@pytest.mark.parametrize('ls1, ls2, result', [([1, 2, 3], [5], (2, 5)),
                                                ([5], [1, 2, 3], (5, 2)), ([5], [5], (5, 5))])
def test_get_one_elemented_lists_averages(ls1, ls2, result):
